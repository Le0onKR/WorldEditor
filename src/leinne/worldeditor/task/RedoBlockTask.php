                    ...
                    if (++$this->pos->z > $this->endPos->z) {
                        ...
                        if (++$this->pos->y > $this->endPos->y) {
                            if ($this->player !== null && !$this->player->isClosed()) {
                                $this->player->sendMessage(WorldEditor::$prefix . "블럭을 이전에 변경했던 상태로 변경했습니다");
                            } else {
                                Server::getInstance()->getLogger()->info(WorldEditor::$prefix . ($this->player !== null ? "{$this->player->getName()}님이 " : "") . "블럭을 이전에 변경했던 상태로 변경했습니다");
                            }
                            break;
                        }
                    }
