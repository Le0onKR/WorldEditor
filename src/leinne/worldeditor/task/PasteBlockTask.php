            ...
            if ($count++ < $worldEditor->getBlockPerTick()) {
                if (($block = array_pop($this->copy)) !== null) {
                    ...
                    if ($this->player !== null && !$this->player->isClosed()) {
                        $this->player->sendMessage(WorldEditor::$prefix . "블럭 붙여넣기가 완료되었습니다");
                    } else {
                        Server::getInstance()->getLogger()->info(WorldEditor::$prefix . ($this->player !== null ? "{$this->player->getName()}님이 명령햔 " : "") . "블럭 붙여넣기가 완료되었습니다");
                    }
                    break;
                }
            } else {
