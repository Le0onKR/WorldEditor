    ...
    public function onRun(): void
    {
        ...
        while (true) {
            if ($count < $worldEditor->getBlockPerTick()) {
                ...
                if (!$before->isSameState($this->block)) {
                    ++$count;
                    ...
                    if (++$this->pos->z > $this->endPos->z) {
                        ...
                        if (++$this->pos->y > $this->endPos->y) {
                            if ($this->player !== null && !$this->player->isClosed()) {
                                $this->player->sendMessage(WorldEditor::$prefix . "블럭 설정이 완료되었습니다");
                            } else {
                                Server::getInstance()->getLogger()->info(WorldEditor::$prefix . ($this->player !== null ? "{$this->player->getName()}님이 명령햔 " : "") . "블럭 설정이 완료되었습니다");
                            }
                            break;
                        }
                    }
